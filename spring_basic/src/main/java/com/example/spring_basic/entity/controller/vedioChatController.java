...
//2023.6.4 用于测试腾讯云的视频聊天功能，现在已经废弃。因为无法生成正确的sig传给前端，前端无法使用。
@Slf4j
@RestController
...
public class vedioChatController {
    @GetMapping("/getSig")
    public String gteSig() {
        //新建一个TLSSigAPIv2对象，用来生成签名
        TLSSigAPIv2 genSigClass = new TLSSigAPIv2(1400813146L, "0b0a9860feb5ccf9f329024f898e5740a2784606620916e2852da9ce631e520b");
        //生成签名
        String userSig = genSigClass.genUserSig("dangren", 36000);
        System.out.println(userSig);
        String testStr = "eJwtzV8LgjAUBfDvsueQzT9rCj1IRQUFRVm*ibW1Luaac2YQffdEfby-czj3i07bo-MWBkXIdTCa9DdwoSzcoWeeK2mEyvirVU9QRWZFbcdizYtca*AoIj7GjHjEp0NioRSdUhYEXhh4ZFDx0WA6p9hnGI8bILsv51V5SOfpNW3auEiwvLnLRSI3oZ5aWsUP6TbVei-Mpd6xGfr9ASM3N-Y_";
        //return userSig;
        return testStr;
    }
    ...
    public class TLSSigAPIv2 {
        ...
        //腾讯提供的主要生成usersig 的方法

        /**
         * sdkappid  应用 SDKAppID，可在即时通信 IM 控制台 的应用卡片中获取。
         * userId    用户 ID，旧称为 Identifier。
         * expire    UserSig 的有效期，单位为秒。
         * userbuf  即时通信 IM 中均默认使用不带 UserBuf 的接口，即该参数默认填写为null。
         * 实时音视频的部分使用场景中可能需要使用带 UserBuf 的接口，例如进房时，具体介绍请参见 进房权限保护。
         * key 密钥信息，可在即时通信 IM 控制台 的应用详情页面中获取，具体操作请参见 获取密钥。
         **/
        private String genUserSig(String userid, long expire, byte[] userbuf) {
            ...
            compressor.end();
            //(2023.5.31)它这个BASe64的工具类未提供，我使用了csdn里其他人给的Base64URL代码
